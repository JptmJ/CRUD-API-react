...
import handleDelete from './DELETE'; // Import handleDelete from DELETE

function GET() {
    const [data, setData] = useState([]);

    useEffect(() => {
        const url = "http://localhost:3010/getGoals";
        fetch(url)
                ...
                <table className="task-table">
                    ...
                    <tbody>
                        {data.map((goalData, index) => (
                            <tr key={index} className={index % 2 === 0 ? "task-row" : ""}>
                                ...
                                <td className="task-cell">
                                    ...
                                    <button onClick={() => handleDelete(goalData.id, data, setData)} className="delete-button">Delete</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
